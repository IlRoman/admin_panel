...
export const CustomDropdownCheckmark = ({
  placeholder,
  name,
  multiSelect,
  formData,
  ...
  const handleChange = (elem) => {
    setSearch(null);
    multiSelect ? null :  setOpen(false)
    //debugger
    onChange(elem);
  };
  ...
  return (
    <div className={`custom-dropdown ${variant} custom-dropdown-checkmark`} >
      <div
          ...
          ? (
            getOptions().map((elem => {
              return (
                ...
                >
                  <span style={{opacity:elem?.check ? 1 : 0.5}}>{elem.name}</span>
                  <CustomCheckbox checked={elem?.check}/>
                </div>
              )
            }))
          )
