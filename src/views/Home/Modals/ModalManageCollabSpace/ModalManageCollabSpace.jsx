...
import './modal.scss';
import { addSpaceCollab, fetchSpaceCollab } from '../../../../crud/spaces/spaces'
...
export const ModalManageCollabSpace = ({ closeModal, current }) => {
  ...
  const loadData = () => {
    if (fetching) {
      ...
      fetchSpaceCollab({ search, id: current?.id, order, orderDirection, page, perPage })
        .then(res => {
  ...
  useEffect(() => {
    ...
    dispatch(showLoaderAction());
    fetchSpaceCollab({ search, id: current?.id, order, orderDirection, page: 1, perPage })
      .then(res => {
  ...
  const handleSubmit = () => {
    dispatch(showLoaderAction());
    addSpaceCollab (current.number,{ collaborators: checked })
      .then(() => {
        dispatch(hideLoaderAction());
        closeModal();
      })
      .catch(err => {
        if (err?.response?.data?.error === 'Not Found') {
          ...
          let errorText =
            err?.response?.data?.message?.includes('not found collaborator with') ?
              err?.response?.data?.message?.match(regExpression) : ''
          if(errorText){
            let findNameCollab = collabList?.find(coll => coll?.id == errorText)
            dispatch(showSimpleModalAction({
              title:'Error',
              text: `Collaborator ${findNameCollab?.fullname || 'user'} could not be added to space because his account has been deleted`
            }));
          }else{
            dispatch(showSimpleModalAction({
              title:'Error',
              text: `Collaborator could not be added to user because his account has been deleted`
            }));
          }
        }
      })
    ...
    <div className="manage-col-modal">
      <form onSubmit={handleSubmit}>
        <CustomModal
          ...
          subtitle={`Check collaborators to assign to ${current?.name}`}
          close={closeModal}
          cancelBtn="Cancel"
          submitBtn="Save"
          submit={handleSubmit}
