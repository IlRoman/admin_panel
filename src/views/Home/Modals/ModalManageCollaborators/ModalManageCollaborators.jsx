...
const tableHead = [
    ...
    {
        title: 'Collaborator Number',
        sortable: true,
        order: 'number',
        position: 'left',
    },
    {
        title: 'Collaborator Address',
        sortable: true,
        order: 'city',
        position: 'left',
    },
...
export const ModalManageCollaborators = ({ closeModal, current }) => {
    ...
    const loadData = () => {
        if (fetching) {
            ...
            fetchCollabs({ search, id: current?.id, order, orderDirection, page, perPage })
                .then(res => {
    ...
    useEffect(() => {
        dispatch(showLoaderAction());
        fetchCollabs({ search, id: current?.id, order, orderDirection, page: 1, perPage })
            .then(res => {
    ...
    const handleSubmit = () => {
        dispatch(showLoaderAction());
        manageCollabs({ id: current.number, collaborators: checked })
            .then(() => {
                dispatch(hideLoaderAction());
                closeModal();
            })
        ...
        <div className="manage-col-modal">
            <form onSubmit={handleSubmit}>
                <CustomModal
                    ...
                    subtitle={`Check collaborators to assign to ${current?.fname} ${current?.lname}`}
                    close={closeModal}
                    cancelBtn="Cancel"
                    submitBtn="Save"
                    submit={handleSubmit}
                                        ...
                                        {collabList.map((elem, index) => {
                                            return (
                                                <div key={index} className='manage-col-modal__table-body-row'>
                                                    ...
                                                    <div className='manage-col-modal__table-body-cell'>
                                                        {`${elem.country} ${elem.city} ${elem.street}`}
                                                    </div>
                                                    <div className='manage-col-modal__table-body-cell'>
                                                        <CustomCheckbox
